...
window.onload = function() {
  ...
  function setReferrerUrl() {
    var referrerUrl = document.referrer || 'homer.html'; // Default to homer.html if no referrer is available
    var destinationUrl = getDestinationUrl(); // Get the dynamically set destination URL
    // Check if the referrer is the dynamically set destination URL, if so, set it to 'homer.html'
    if (referrerUrl.includes(destinationUrl)) {
      referrerUrl = 'homer.html';
    }
